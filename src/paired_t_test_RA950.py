        ...
        for j in range(len(slice_thicknesses)):
            for k in range(len(kernels)):
                ...
                if np.isnan(result_1_sample[0]):
                    nan_flag=True
                    result_1_sample=scipystats.ttest_1samp(curr_ra950,0.05,nan_policy='omit')
                    
                conf=scipystats.t.interval(.95,len(curr_ra950)-1,loc=curr_ra950.mean(skipna=True),scale=scipystats.sem(curr_ra950,nan_policy='omit'))
                if __DEBUG__:
